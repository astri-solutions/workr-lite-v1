...
│       │   └── _headers    # Cloudflare Pages response headers (noindex while in test phase)
│       └── package.json
```
...
- **Deploy**: Cloudflare Pages — deploys automatically on push to `main`, at the apex domain `workr.dev.br`
...
- **Admin panel** (`workr-lite-v1`): deployed on Cloudflare Pages, at the apex domain `workr.dev.br`
- **Each client portal**: gets its own GitHub repo (`astri-solutions/workr-portal-{subdomain}`) generated from `cliente-workr-lite` template, and its own Cloudflare Pages **project**, reachable at `<cliente>.workr.dev.br` (custom domain attached via the Cloudflare API, backed by a CNAME to the project's `*.pages.dev` subdomain)
- **Why separate Cloudflare Pages projects per portal**: `cliente-workr-lite` is a static HTML site, independent from the admin SPA (React) — each client portal gets its own repo/project/deploy pipeline so portals redeploy independently of each other and of the admin panel.
- Cloudflare Pages project creation (and custom-domain/DNS attachment) is automatic during portal provisioning, but **requires `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` secrets** to be set in Supabase Edge Function secrets. Without them, the repo is created but Cloudflare Pages deployment must be set up manually.
- Vercel is not used anywhere in this project — not for portals, not for the CMS. Only GitHub, Cloudflare (Pages + DNS), and Supabase.
...
| `CLOUDFLARE_API_TOKEN` | Supabase Edge Function secrets | Create Cloudflare Pages project + custom domain/DNS per portal |
| `CLOUDFLARE_ACCOUNT_ID` | Supabase Edge Function secrets | Cloudflare account that owns the Pages projects and the `workr.dev.br` zone |
...
When migrating from Cloudflare/Supabase to a dedicated server:
